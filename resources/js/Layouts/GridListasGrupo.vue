    ...
    <div>
        <div class="mb-6">
            ...
            <div class="mt-4 text-sm text-gray-600">
                <span>Mostrando {{ listas?.data?.length || 0 }} de {{ listas?.total || 0 }} listas</span>
            </div>
        </div>
        ...
        <div v-if="(listas?.data?.length || 0) === 0" class="text-center py-12">
        ...
        <div v-if="(listas?.last_page || 0) > 1" class="mt-8 flex items-center justify-between">
            <div class="flex-1 flex justify-between sm:hidden">
                <button @click="goToPage((listas?.current_page || 1) - 1)" :disabled="(listas?.current_page || 1) === 1"
                    class="relative inline-flex items-center px-4 py-2 border border-gray-300 text-sm font-medium rounded-md text-gray-700 bg-white hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
                    Anterior
                </button>
                <button @click="goToPage((listas?.current_page || 1) + 1)"
                    :disabled="(listas?.current_page || 1) === (listas?.last_page || 1)"
            ...
            <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                <div>
                    <p class="text-sm text-gray-700">
                        Mostrando
                        <span class="font-medium">{{ listas?.from || 0 }}</span>
                        a
                        <span class="font-medium">{{ listas?.to || 0 }}</span>
                        de
                        <span class="font-medium">{{ listas?.total || 0 }}</span>
                        resultados
                    </p>
                </div>
                <div>
                    <nav class="relative z-0 inline-flex rounded-md shadow-sm -space-x-px" aria-label="Pagination">
                        <button @click="goToPage((listas?.current_page || 1) - 1)"
                            :disabled="(listas?.current_page || 1) === 1"
...
const visiblePages = computed(() => {
    const pages = []
    const total = props.listas?.last_page || 1
    const current = props.listas?.current_page || 1
...
const goToPage = (page) => {
    if (page >= 1 && page <= (props.listas?.last_page || 1)) {
        router.get(route('listas.index'), {
